...
export default function FreeDeliveryBanner() {
  ...
  const viganBanner = (
    <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 text-white py-3 px-4 shadow-md">
      <div className="container mx-auto flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
        <div className="flex items-center gap-2 font-extrabold text-base sm:text-lg">
          <span>CVN&apos;EAT debarque au Vigan</span>
        </div>
        <div className="flex items-center gap-2 bg-white/15 px-3 py-1 rounded-full text-sm font-semibold">
          <FaTruck className="w-4 h-4" />
          <span>Livraison fixe 10EUR</span>
        </div>
        <div className="bg-yellow-400 text-slate-900 px-3 py-1 rounded-full text-sm font-bold">
          Minimum commande 20EUR
        </div>
      </div>
    </div>
  );

  if (!isPromoActive) return viganBanner;
