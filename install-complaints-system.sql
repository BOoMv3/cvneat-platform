-- ========================================
-- SCRIPT D'INSTALLATION COMPLET - SYSTÈME DE RÉCLAMATIONS
-- ========================================
-- À exécuter dans Supabase SQL Editor

-- 1. Créer la table des réclamations
CREATE TABLE IF NOT EXISTS complaints (
    ...
    -- Preuves
    evidence_images JSONB, -- Stocke les URLs des images
...
-- 2. Créer la table pour l'historique des réclamations client (anti-fraude)
CREATE TABLE IF NOT EXISTS customer_complaint_history (
    ...
    trust_score INTEGER DEFAULT 100, -- Score de 0 à 100
    is_flagged BOOLEAN DEFAULT FALSE,
    flag_reason TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
...
-- 3. Créer la table pour les preuves de réclamation
...
-- 4. Créer la table pour les feedbacks clients (approche positive)
...
-- 6. Fonction pour valider la fenêtre de temps des réclamations
...
BEGIN
    ...
    BEGIN
        ...
        IF hours_diff < 1 THEN
            RAISE EXCEPTION 'Réclamation trop tôt: minimum 1 heure après la livraison (basé sur la création de commande)';
        END IF;
        
        IF hours_diff > 48 THEN
            RAISE EXCEPTION 'Réclamation trop tardive: maximum 48 heures après la livraison (basé sur la création de commande)';
        END IF;
    END;
...
-- 7. Fonction pour mettre à jour l'historique des réclamations
...
-- 8. Fonction pour convertir un feedback en réclamation si nécessaire
...
-- 11. Créer les policies pour complaints
...
CREATE POLICY "Restaurants can view complaints for their orders"
    ON complaints FOR SELECT
    USING (EXISTS (
...
-- 12. Créer les policies pour customer_complaint_history
...
-- 13. Créer les policies pour complaint_evidence
...
-- 14. Créer les policies pour order_feedback
