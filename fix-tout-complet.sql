...
-- 1. Corriger le compte livreur1
SELECT '=== CORRECTION LIVREUR1 ===' as info;
UPDATE users 
SET role = 'delivery',
    password = 'livreur123',
    updated_at = NOW()
WHERE email = 'livreur1@cvneat.com';

-- 2. Corriger le compte livreur
SELECT '=== CORRECTION LIVREUR ===' as info;
UPDATE users 
SET role = 'delivery',
    password = 'livreur123',
    updated_at = NOW()
...
SELECT '=== VÉRIFICATION CORRECTIONS ===' as info;
SELECT id, email, role, nom, prenom, password FROM users WHERE email IN ('livreur1@cvneat.com', 'livreur@cvneat.com');

-- 4. Supprimer les anciennes commandes de test
SELECT '=== SUPPRESSION ANCIENNES COMMANDES ===' as info;
DELETE FROM orders WHERE customer_name IN ('Marie Dupont', 'Jean Martin', 'Sophie Leroy');

-- 5. Créer des commandes de test avec un restaurant existant
SELECT '=== CRÉATION COMMANDES DE TEST ===' as info;
INSERT INTO orders (
  restaurant_id,
  customer_name,
  customer_phone,
...
-- Commande 4: Commande prête pour livreur
(
  (SELECT id FROM restaurants LIMIT 1),
  'Pierre Durand',
  '0666123456',
