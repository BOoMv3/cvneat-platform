...
async function markBonnePate439Paid() {
  console.log('🔄 Marquage des commandes de La Bonne Pâte correspondant au virement de 439€...\n');
  ...
  try {
    ...
    // 2. Récupérer toutes les commandes non payées au restaurant
    ...
    if (ordersToMark.length === 0) {
      console.log('ℹ️  Aucune commande à marquer\n');
      return;
    }
    ...
    const remainingTotal = (remainingOrders || []).reduce((sum, o) => sum + parseFloat(o.total || 0), 0);
    console.log(`📊 Reste à payer: ${remainingTotal.toFixed(2)}€\n`);
