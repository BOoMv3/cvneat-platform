...
export default function AppAutoRedirect() {
  ...
  useEffect(() => {
    if (!isCapacitorApp()) return;
    if (!pathname || (pathname !== '/' && pathname !== '/app-welcome')) return;
    ...
    async function run() {
      ...
      // 1) Essayer via API (fiable, bypass RLS côté serveur)
      let role = '';
      try {
        const res = await fetch('/api/users/me', {
          headers: { Authorization: `Bearer ${session.access_token}` },
      ...
      // 3) Si toujours vide, essayer “restaurant” par présence d'une fiche restaurant
      if (!role) {
        try {
          const byUser = await supabase
            .from('restaurants')
