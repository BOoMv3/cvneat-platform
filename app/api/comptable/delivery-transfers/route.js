...
export async function GET(request) {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select(
        'id, delivery_id, delivery_name, delivery_email, amount, transfer_date, reference_number, period_start, period_end, orders_count, status, notes, created_at'
      )
      .eq('status', 'completed')
    ...
    if (q) {
      rows = rows.filter(
        (r) =>
          ...
          (r.delivery_email || '').toLowerCase().includes(q) ||
    ...
    console.error('comptable/delivery-transfers:', error);
