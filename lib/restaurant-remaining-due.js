...
export function isBonnePateRestaurant(nom) {
  const n = (nom || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
...
/**
 * Part du virement Bonne Pâte du 29/07/2026 qui rembourse un livreur payé de poche
 * (repassé sur CVN'EAT), pas un reversement restaurant.
 */
export const BONNE_PATE_POCKET_DRIVER_REIMBURSEMENT_EUR = 56.2;
...
export function sumRelevantTransfers(transfers, restaurant) {
  ...
  if (!is99StreetFoodRestaurant(restaurant?.nom)) {
    total = round2(list.reduce((sum, t) => sum + (parseFloat(t.amount || 0) || 0), 0));
  } else {
    total = round2(
      list
  ...
  // Bonne Pâte : retirer le remboursement livreur poche inclus dans un virement resto
  if (isBonnePateRestaurant(restaurant?.nom)) {
    total = Math.max(0, round2(total - BONNE_PATE_POCKET_DRIVER_REIMBURSEMENT_EUR));
  }
...
export function computeRestaurantRemainingDue({
  ...
  return {
    ...
    is99StreetFood: is99,
    isBonnePate: isBonnePateRestaurant(restaurant?.nom),
    hasManualOverride,
  };
}
