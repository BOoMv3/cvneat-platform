...
/**
 * Plugin natif Capacitor (imprimante intégrée Sunmi).
 * Capacitor 5/7 : il faut registerPlugin — Plugins.SunmiPrint n'existe pas tout seul.
 */
async function getSunmiPrintPlugin() {
  if (typeof window === 'undefined') return null;

  try {
    const core = await import('@capacitor/core');
    if (typeof core.registerPlugin === 'function') {
      return core.registerPlugin('SunmiPrint');
    }
  } catch (e) {
    console.warn('[print] registerPlugin unavailable:', e?.message || e);
  }

  try {
    return window.Capacitor?.Plugins?.SunmiPrint || null;
  } catch (_) {
    return null;
  }
}
...
/**
 * Impression partenaire : Sunmi intégré (app Android Capacitor) prioritaire.
 * Le navigateur Chrome seul NE PEUT PAS piloter l'imprimante AIDL Sunmi.
 * @returns {{ ok: boolean, method: 'sunmi'|'rawbt'|null, error?: string }}
 */
export async function printPartnerTicket(receiptText) {
  ...
  // 1) App Capacitor Android → imprimante intégrée Sunmi
  try {
    const plugin = await getSunmiPrintPlugin();
    const native = isCapacitorNative();

    if (plugin && typeof plugin.printText === 'function') {
      let available = native; // si on est natif, on tente même si isAvailable échoue
      if (typeof plugin.isAvailable === 'function') {
        try {
          const info = await plugin.isAvailable();
          if (info && typeof info.available === 'boolean') {
            available = info.available === true || native;
          }
        } catch (_) {
          available = native;
        }
    ...
    if (!native) {
      return {
        ...
        method: null,
        error:
          "L'impression Sunmi marche dans l'app CVN'EAT (pas dans Chrome). Ouvre l'application installée, puis réessaie.",
      };
    }
    ...
    return {
      ...
      method: null,
      error:
        "Plugin Sunmi introuvable dans cette APK. Rebuild l'app Android (SunmiPrint) puis réinstalle.",
    };
  } catch (e) {
    console.warn('[print] Sunmi erreur:', e?.message || e);
    // Si on est natif et que Sunmi a planté, on peut tenter RawBT en secours
    if (!isCapacitorNative()) {
      return {
        ok: false,
        method: null,
        error: e?.message || 'Erreur impression Sunmi',
      };
    }
  }

  // 2) Fallback RawBT (imprimante Bluetooth externe) — seulement si installé
  try {
    const launched = await printWithRawBt(text);
    if (launched) {
      return {
        ok: true,
        method: 'rawbt',
        warning: 'Tentative RawBT (vérifie que RawBT est installé).',
      };
    }
  } catch (_) {}
  ...
  return {
    ok: false,
    method: null,
    error: "Impression impossible (imprimante Sunmi / RawBT).",
  };
}
