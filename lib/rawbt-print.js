...
/**
 * Impression partenaire : Sunmi intégré (app Android) prioritaire, sinon RawBT.
...
export async function printPartnerTicket(receiptText) {
  ...
  // 1) Sunmi V2 Pro (imprimante intégrée) via plugin Capacitor natif
  try {
    const cap = window.Capacitor;
    const plugin = cap?.Plugins?.SunmiPrint;
    const isAndroid =
      (typeof cap?.getPlatform === 'function' && cap.getPlatform() === 'android') ||
      (typeof cap?.isNativePlatform === 'function' &&
        cap.isNativePlatform() &&
        String(cap?.getPlatform?.() || '').toLowerCase() === 'android');

    if (plugin && (isAndroid || plugin.isAvailable)) {
      let available = true;
      if (typeof plugin.isAvailable === 'function') {
        const info = await plugin.isAvailable();
        available = info?.available === true;
      }
      if (available && typeof plugin.printText === 'function') {
        await plugin.printText({ text: String(receiptText || '') });
        return { ok: true, method: 'sunmi' };
      }
    }
  } catch (e) {
    console.warn('[print] Sunmi indisponible:', e?.message || e);
  }

  // 2) Fallback RawBT (imprimante Bluetooth externe)
  const launched = await printWithRawBt(receiptText);
  if (launched) return { ok: true, method: 'rawbt' };
  return {
    ok: false,
    method: null,
    error: "Impression impossible (Sunmi / RawBT).",
  };
}
