    ...
    for (const order of expiredOrders) {
      try {
        ...
        if (order.payment_status === 'paid' && order.stripe_payment_intent_id && orderTotal > 0 && stripe) {
          try {
            // RE-VÉRIFIER LE STATUT juste avant le remboursement pour éviter les race conditions
            ...
            // VÉRIFICATION CRITIQUE: Ne pas rembourser si la commande est déjà acceptée, en livraison ou livrée
            if (!currentOrder) {
              console.log(`⚠️ Commande ${order.id?.slice(0, 8)} introuvable, annulation du nettoyage.`);
              continue;
            }
            
            if (currentOrder.livreur_id !== null) {
              console.log(`⚠️ Commande ${order.id?.slice(0, 8)} a été acceptée par un livreur entre-temps, annulation du nettoyage.`);
              continue;
            }
            
            if (currentOrder.statut !== 'en_attente') {
              // Vérifier spécifiquement les statuts qui indiquent que la commande est en cours ou livrée
              if (currentOrder.statut === 'livree' || currentOrder.statut === 'delivered' || 
                  currentOrder.statut === 'en_livraison' || currentOrder.statut === 'in_delivery' ||
                  currentOrder.statut === 'en_preparation' || currentOrder.statut === 'pret_a_livrer') {
                console.log(`⚠️ Commande ${order.id?.slice(0, 8)} est déjà en cours ou livrée (statut: ${currentOrder.statut}), annulation du nettoyage.`);
                continue;
              }
              // Pour les autres statuts (comme 'annulee'), on continue le nettoyage normalement
            }
