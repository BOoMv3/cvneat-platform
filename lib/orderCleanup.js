      ...
      try {
        ...
        if (order.payment_status === 'paid' && order.stripe_payment_intent_id && orderTotal > 0 && stripe) {
          try {
            ...
            if (currentOrder && (currentOrder.livreur_id !== null || currentOrder.statut !== 'en_attente')) {
              console.log(`⚠️ Commande ${order.id?.slice(0, 8)} a été acceptée entre-temps, annulation du nettoyage.`);
              continue;
            }
