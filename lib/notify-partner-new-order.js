...
export async function notifyPartnerNewOrder(order, { supabaseAdmin, origin } = {}) {
  ...
  if (supabaseAdmin) {
    ...
    // Retrait : l'admin est informé, les livreurs ne reçoivent rien.
    if (pickup) {
      try {
        await sendDeliveryAppPush({
          orderId: order.id,
          total: totalWithDelivery,
          title: 'Retrait sur place 🛍️',
          body: `Commande #${order.id.slice(0, 8)} - ${totalWithDelivery}€ — pas de livreur`,
          audience: 'admins',
          data: {
            type: 'pickup_order',
            orderId: order.id,
        ...
        console.warn('⚠️ notifyPartnerNewOrder admin pickup:', e?.message || e);
      }
    }
  }
