...
export async function generatePasswordRecoveryUrl(email) {
  ...
  /** Page dédiée : le token n’est consommé qu’à la soumission (évite scanners mail / PKCE). */
  const resetPage = `${base}/auth/reset-link`;
  const redirectTo = resetPage;
  ...
  if (tokenHash) {
    ...
    return `${resetPage}?${params.toString()}`;
  }
