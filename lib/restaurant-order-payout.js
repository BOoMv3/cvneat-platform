...
export function computeOrderRestaurantPayoutEur(order, restaurant) {
  ...
  // Taux fixe (Bonne Pâte 0%, All'ovale 15%) : toujours recalculer.
  // Des commandes historiques ont un restaurant_payout / commission stockés incohérents.
  if (fixed === 0) {
    return round2(subtotal + subsidy);
  }
  if (fixed != null) {
    const { payout } = computeCommissionAndPayout(subtotal, fixed);
...
export function computeOrderCommissionEur(order, restaurant) {
  ...
  if (fixed === 0) return 0;
  if (fixed != null) {
    return computeCommissionAndPayout(subtotal, fixed).commission;
  }
