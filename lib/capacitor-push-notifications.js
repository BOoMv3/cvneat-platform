...
import { dispatchPartnerNewOrderFromPushData } from './partner-order-alert-bridge';
...
export const initPushNotifications = async () => {
  ...
  try {
    ...
    PushNotifications.addListener('registration', (token) => {
      ...
      saveTokenToServer(token.value, { force: true });
    });
    ...
    PushNotifications.addListener('pushNotificationReceived', (notification) => {
      ...
      // Relier le push système à la popup in-app partenaire (Sunmi : Realtime/SSE souvent KO)
      const data = notification?.data || notification?.notification?.data || {};
      // Retrait / notif admin : ne jamais déclencher l'alerte partenaire livreur
      if (data.type === 'new_order_available' || data.type === 'pickup_order') {
        return;
      }
      dispatchPartnerNewOrderFromPushData(data).catch((e) => {
        console.warn('Push → popup partenaire:', e?.message || e);
      });
    });
    ...
    try {
      ...
      const tryAssociateStoredToken = async (reason) => {
        try {
          ...
          if (storedToken) {
            ...
            await saveTokenToServer(storedToken, { force: true });
          }
        } catch (e) {
          // ignore
        }
      ...
      // iOS/Android: au retour au premier plan, forcer une réinscription push
      // pour corriger les cas où le token n'est plus valide après update/réinstall.
      try {
        ...
        App.addListener('appStateChange', async ({ isActive }) => {
          ...
          try {
            ...
            console.warn('⚠️ Réinscription push en reprise échouée:', e?.message || e);
          }
        });
      } catch (e) {
        // Plugin App non dispo (web), ignorer.
      }
...
const saveTokenToServer = async (token, options = {}) => {
  try {
    const forceAssoc = options?.force === true;
...
// Obtenir la plateforme (android/ios/web) — toujours lowercase pour Supabase CHECK
const getPlatform = () => {
  try {
    if (typeof window === 'undefined') return 'web';
    const p = (window.Capacitor?.getPlatform?.() || 'web').toString().toLowerCase();
    if (p && p !== 'web') return p;
    if (isNativeApp()) {
...
const handleNotificationClick = (notification) => {
  ...
  // Popup + navigation si nouvelle commande partenaire
  if (data.type === 'new_order') {
    dispatchPartnerNewOrderFromPushData(data).catch(() => {});
  }
  let targetUrl = '';
  ...
  } else if (data.type === 'pickup_order') {
    targetUrl = data.url || '/admin/orders';
  } else if (data.type === 'delivery_available' || data.type === 'new_order_available') {
    targetUrl = data.url || '/delivery/dashboard';
  } else if (
    data.type === 'payment_made'
  ) {
    targetUrl = data.url || '/delivery/factures';
  } else if (
    data.type === 'delivery_message' ||
    data.type === 'admin' ||
    data.type === 'system' ||
    data.type === 'missing_docs' ||
    data.type === 'admin_message' ||
    data.type === 'admin_dm'
  ) {
    targetUrl = data.url || '/delivery/messages';
  } else if (data.type === 'delivery_dm') {
    targetUrl = data.url || '/delivery/messages?tab=dm';
