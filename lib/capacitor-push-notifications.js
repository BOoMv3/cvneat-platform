...
const getPlatform = () => {
  ...
  // Fallback robuste: parfois getPlatform() retourne "web" même dans une WebView Capacitor (remote URL).
  // Or on a BESOIN de 'ios'/'android' pour que /api/notifications/send-push cible bien la plateforme.
  if (isNativeApp()) {
...
const handleNotificationClick = (notification) => {
  ...
  // Priorité 2: rediriger selon le type de notification
  if (data.type === 'new_order') {
    window.location.href = '/partner/orders';
    return;
  }
  if (data.type === 'delivery_available' || data.type === 'new_order_available') {
    window.location.href = '/delivery/dashboard';
    return;
  }
  
  // Priorité 3: notification avec orderId
  if (data.orderId) {
