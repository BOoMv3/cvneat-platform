...
export const sendAPNsNotification = async (deviceToken, title, body, data = {}) => {
  ...
  // iOS 13+ exige apns-push-type. node-apn 2.2.0 ne l’envoie pas → notifs silencieusement droppées.
  notification.pushType = 'alert';
  notification.priority = 10;
  const originalHeaders = notification.headers.bind(notification);
  ...
  const sendWithEnv = async (env) => {
    ...
    try {
      ...
      if (isServerless && typeof provider.shutdown === 'function') {
        try {
          const maybePromise = provider.shutdown();
          // Supporter aussi le cas où shutdown renvoie une Promise
          if (maybePromise && typeof maybePromise.then === 'function') {
            await maybePromise;
          }
        } catch (_) {
          // Ignorer les erreurs de shutdown, non bloquant
        }
      }
    }
  };
