...
export function buildSampleRestaurantInvoicePayload() {
  ...
  const transfer = {
    ...
    notes: 'Document d’exemple — ventes via CVN’EAT, commission plateforme 20 % sur le CA articles (hors frais de livraison), net viré au restaurant.',
  };
...
export async function buildSampleRestaurantInvoicePdfBuffer(logoBuffer = null) {
  const data = buildSampleRestaurantInvoicePayload();
