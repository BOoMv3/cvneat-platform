...
} from './livreur-delivery-earnings.js';
...
export function buildDeliveryInvoiceReference(transfer) {
  const dateStr = formatDateFR(transfer.transfer_date).replace(/\//g, '');
  return `CVNEAT-LIV-${dateStr}-${(transfer.id || '').slice(0, 6).toUpperCase()}`;
}

export function buildDeliveryBeneficiaryLines(driver, transfer) {
  const displayName =
    driver?.legal_name ||
  ...
  return [
    ...
    driver?.vat_number ? `TVA : ${driver.vat_number}` : null,
    transfer?.delivery_email || driver?.email
      ? `Email : ${transfer?.delivery_email || driver?.email}`
      : null,
    driver?.adresse || driver?.ville || driver?.code_postal
      ? `${driver?.adresse || ''} ${driver?.code_postal || ''} ${driver?.ville || ''}`.trim()
      : null,
  ].filter(Boolean);
}

export const CVNEAT_ISSUER_LINES = [
  "CVN'EAT (SAS)",
...
export function buildDeliveryInvoiceHtml({
  issuerLines = CVNEAT_ISSUER_LINES,
  deliveryLines,
  transfer,
  reference,
  ordersDetail,
...
/**
 * Charge transfer + driver + orders et renvoie HTML + meta.
 */
export async function loadDeliveryTransferInvoice(supabaseAdmin, transferId) {
  ...
  let { data: driver } = await supabaseAdmin
    .from('users')
    .select('id, nom, prenom, email, adresse, code_postal, ville, siret, legal_name, vat_number')
  ...
  if (!driver) {
    const fallback = await supabaseAdmin
      .from('users')
      .select('id, nom, prenom, email, adresse, siret, legal_name')
      .eq('id', transfer.delivery_id)
      .single();
    driver = fallback.data;
  }

  const deliveryLines = buildDeliveryBeneficiaryLines(driver, transfer);
  const reference = buildDeliveryInvoiceReference(transfer);
  ...
  return { transfer, driver, reference, html, ordersDetail, ordersCount };
}
