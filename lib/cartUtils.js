...
/**
 * Indique si le panier contient au moins un article marqué alcool côté menu (contains_alcohol).
 * @param {Array} cartItems
 * @param {Array} menuItems — menu restaurant (champ contains_alcohol après migration)
 */
export function cartHasAlcohol(cartItems = [], menuItems = []) {
  ...
  const alcoholById = new Map(
    menuItems.map((m) => [m.id, m.contains_alcohol === true || m.contains_alcohol === 'true'])
  );

  const idIsAlcohol = (id) => (id ? alcoholById.get(id) === true : false);

  const walk = (item) => {
    if (!item) return false;
    if (idIsAlcohol(item.id)) return true;
    if (idIsAlcohol(item.selected_drink?.id)) return true;
    if (Array.isArray(item.formula_items)) {
      for (const fi of item.formula_items) {
        if (walk(fi)) return true;
        if (idIsAlcohol(fi.id) || idIsAlcohol(fi.menu_id)) return true;
      }
    }
    const details = item.customizations?.formula_items_details;
    if (Array.isArray(details)) {
      for (const fi of details) {
        if (walk(fi)) return true;
        if (idIsAlcohol(fi.id) || idIsAlcohol(fi.menu_id) || idIsAlcohol(fi.menu?.id)) return true;
      }
    }
    return false;
  };
