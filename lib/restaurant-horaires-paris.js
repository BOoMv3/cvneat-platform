...
/**
 * `is_closed` en base est parfois `1` / `"1"` / `"true"` ; l’API `/hours` utilisait `!== true` → incohérence avec ce moteur.
 */
export function isHorairesClosedTruthy(v) {
...
/** Prochaine date (max 21 j) dont le jour calendaire Paris = `dayIndex` (0=dimanche … 6=samedi). */
export function pickRefDateForParisDayIndex(dayIndex, anchor = new Date()) {
  const target = Number(dayIndex);
  if (!Number.isFinite(target) || target < 0 || target > 6) return anchor;
  for (let i = 0; i < 21; i += 1) {
    const t = new Date(anchor.getTime() + i * 86400000);
    if (getParisNow(t).dayIndex === target) return t;
  }
  return anchor;
}
...
export function isRestaurantOpenNowFromHoraires(horairesRaw, refDate = new Date()) {
  ...
  // Fermeture explicite sans créneau : jour fermé. Avec plages / plage unique, on suit les heures (données souvent incohérentes : is_closed:1, ouvert:false, etc.).
  if (isHorairesClosedTruthy(day.is_closed) && !hasPlages && !hasSingle) return false;

  // `ouvert: false` sans aucun créneau = jour fermé. Avec plages / plage unique, on suit les heures
  // (beaucoup de fiches ont ouvert:false + plages en base → sinon liste « Ouvert » vs fiche « Fermé »).
  if ((day.ouvert === false || day.open === false) && !hasPlages && !hasSingle) return false;
