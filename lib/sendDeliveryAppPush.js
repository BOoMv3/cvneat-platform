...
/** Push ciblé vers une liste d'user_id (device_tokens APNs/FCM). */
export async function sendPushToUserIds(userIds, title, body, data = {}) {
  const ids = [...new Set((userIds || []).filter(Boolean))];
  if (!ids.length) return { sent: 0, total: 0, message: 'Aucun destinataire' };
  const tokens = await getTokensForUserIds(ids);
  if (!tokens.length) return { sent: 0, total: 0, message: 'Aucun token trouvé', targets: ids.length };
  // FCM exige souvent des strings dans data
  const payload = {};
  for (const [k, v] of Object.entries(data || {})) {
    if (v == null) continue;
    payload[k] = typeof v === 'string' ? v : String(v);
  }
  return sendPushToTokens(tokens, title, body, payload);
}

export async function sendDeliveryAppPush({
  orderId,
  total,
  title,
  body,
  data = {},
  /** 'all' = livreurs + admins (livraison). 'admins' = admin seulement (retrait). 'drivers' = livreurs seulement. */
  audience = 'all',
} = {}) {
  const t = title || 'Nouvelle commande disponible 🚚';
  const shortId = orderId ? String(orderId).slice(0, 8) : null;
  // Livreurs : jamais le montant client — juste savoir qu’il y a une course
  const bodyDriver = shortId
    ? `Nouvelle course #${shortId} — ouvre l’app pour accepter`
    : 'Une nouvelle course t’attend';
  // Admin : montant utile pour le suivi ops (si fourni)
  const totalLabel =
    total == null || total === ''
      ? null
  ...
  const bodyAdmin =
    body ||
    (shortId && totalLabel
      ? `Commande #${shortId} - ${totalLabel}`
      : bodyDriver);

  const baseType = data.type || 'new_order_available';
  const baseOrderId = data.orderId || orderId;
  const who = ['admins', 'drivers', 'all'].includes(audience) ? audience : 'all';

  try {
    const deliveryIds = who === 'admins' ? [] : await fetchUserIdsForRoles(['delivery', 'livreur']);
    const adminIds = who === 'drivers' ? [] : await fetchUserIdsForRoles(['admin']);
    ...
    if (deliveryOnlyIds.length === 0 && adminIds.length === 0) {
      console.warn('⚠️ [sendDeliveryAppPush] Aucun destinataire (audience=', who, ')');
    ...
    // Ne pas exposer le total dans le data payload livreur
    delete payloadDriver.total;
    delete payloadDriver.amount;
    ...
    const rDel = await sendPushToTokens(tokensDelivery, t, bodyDriver, payloadDriver);
    const rAdm = await sendPushToTokens(tokensAdmin, t, bodyAdmin, payloadAdmin);

    const sent = rDel.sent + rAdm.sent;
    const totalTokens = rDel.total + rAdm.total;
    const errors = [...(rDel.errors || []), ...(rAdm.errors || [])];

    console.log(
      `✅ [sendDeliveryAppPush] Envoyé: ${sent}/${totalTokens} (livreurs-only: ${tokensDelivery.length} tok, admins: ${tokensAdmin.length} tok)`
    );
    return { sent, total: totalTokens, errors };
  } catch (error) {
