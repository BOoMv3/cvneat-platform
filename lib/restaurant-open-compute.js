...
export function normalizeRestaurantOpenFields(restaurantLike, now = new Date()) {
  ...
  // Toujours dériver des plages (ou fermé si pas d’entrée valable pour aujourd’hui) ; ouvert_manuellement seul peut forcer.
  const isOpenNow = om === true || isOpenNowFromHoraires(restaurantLike.horaires, now);
