...
export function normalizeRestaurantOpenFields(restaurantLike, now = new Date()) {
  ...
  // Sans horaires configurés : fermé. Avec horaires : plages uniquement.
  // ouvert_manuellement ne force l’ouverture que s’il n’y a pas de plage explicite (sinon un flag oublié = ouvert toute la nuit).
  ...
  const explicit = hasH ? hasExplicitScheduleForDay(restaurantLike.horaires, now) : false;
  const isOpenNow = fm
    ? false
    : om
      ? explicit
