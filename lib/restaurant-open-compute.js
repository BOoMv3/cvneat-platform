...
export function normalizeRestaurantOpenFields(restaurantLike, now = new Date()) {
  ...
  // Sans horaires en base : on ne force pas « fermé » (comportement proche de l’ancien mode « pas fermé manuellement »).
  const hasH = hasHorairesConfigured(restaurantLike.horaires);
  const isOpenNow = om === true || (hasH ? isOpenNowFromHoraires(restaurantLike.horaires, now) : true);
