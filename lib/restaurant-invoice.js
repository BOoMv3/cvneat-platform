...
export const CVNEAT_INVOICE_PARTY_LINES = [
  "CVN'EAT (SAS)",
...
function displayTransferNotes(notes) {
  if (!notes) return null;
  const { text } = parseTransferNotes(notes);
  return text;
}

export function buildRestaurantTransferInvoiceHtml({ restaurant, transfer, orders, totals, invoiceNumber, options = {} }) {
  const rowsHtml = (orders || [])
    .map((o) => {
      const commission = computeTransferOrderCommissionEur(o, restaurant);
      const payout = computeTransferOrderPayoutEur(o, restaurant);
      const articles = getOrderArticlesSubtotalEur(o);
      return `
        <tr>
          <td>${formatDateFR(o.created_at)}</td>
          <td>${(o.id || '').slice(0, 8)}</td>
          <td style="text-align:right">${formatEUR(articles)}</td>
  ...
  const clientLines = CVNEAT_INVOICE_PARTY_LINES;
  ...
  const notesText =
    displayTransferNotes(transfer.notes) ||
    (transfer.notes && !String(transfer.notes).trim().startsWith('{')
      ? String(transfer.notes).trim()
      : '');

  const sampleBannerBlock = options.sampleBanner
    ? `<div style="background:#fef3c7;border:2px solid #f59e0b;border-radius:10px;padding:12px 14px;margin-bottom:16px;font-size:13px;color:#92400e;line-height:1.45;">
        <strong>⚠ DOCUMENT D'EXEMPLE — DONNÉES FICTIVES</strong><br/>
        Ce document illustre le relevé de ventes transmis aux restaurants partenaires (restaurant émetteur, CVN'EAT client).
        Montants, SIRET, références et commandes sont fictifs.
      </div>`
    : '';

  const clientLogoBlock = options.logoSrc
    ? `<img src="${options.logoSrc}" alt="CVN'EAT" style="height:48px;width:auto;margin-bottom:8px;" />`
    : '';
...
<html lang="fr">
  ...
  <body style="background: #fff; color: #111;">
    ...
    <h1>Relevé de ventes et reversement</h1>
    <h2>Virement enregistré — ${invoiceNumber ? `N° ${invoiceNumber}` : 'numéro en attente'}</h2>

    <div class="grid">
      <div class="card">
        <h3>Émetteur (restaurant)</h3>
        ${issuerLines.map((x) => `<div class="line">${x}</div>`).join('')}
      </div>
      <div class="card">
        <h3>Client / destinataire (plateforme)</h3>
        ${clientLogoBlock}
        ${clientLines.map((x) => `<div class="line">${x}</div>`).join('')}
      </div>
    </div>
    ...
    <div class="card">
      ...
      ${notesText ? `<div class="meta">${notesText}</div>` : ''}
    </div>
    ...
    <div class="meta">
      Relevé émis par le restaurant partenaire — reversement du net après commission CVN'EAT.
      Exonération de TVA CVN'EAT, art. 293 B du CGI (à valider avec votre expert-comptable).
    </div>
  </body>
</html>`;
}
