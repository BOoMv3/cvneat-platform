...
export function buildRestaurantTransferInvoiceHtml({ restaurant, transfer, orders, totals, invoiceNumber }) {
  ...
  const adjustmentRowHtml = showPayoutAdjustment
    ? `<tr>
          <td colspan="4" class="right">Ajustement (alignement virement)</td>
          <td class="right">${formatEUR(payoutDelta)}</td>
        </tr>`
    : '';
...
<html lang="fr">
  ...
  <body style="background: #fff; color: #111;">
    ...
    <table>
      ...
      <tfoot>
        <tr>
          <td colspan="2" class="right">Totaux commandes</td>
        ...
        ${adjustmentRowHtml}
        <tr>
          <td colspan="4" class="right">Net viré (référence paiement)</td>
          <td class="right">${formatEUR(footerPayoutEur)}</td>
        </tr>
      </tfoot>
    </table>
