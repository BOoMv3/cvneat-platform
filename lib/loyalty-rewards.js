...
const toPositiveEur = (value, fallback) => {
...
/**
 * Montant max ajouté au reversement restaurant quand la récompense « article offert » est utilisée.
 * Par défaut: 3.00€ (boisson/dessert), ajustable via ENV côté front/build.
 */
export const LOYALTY_ARTICLE_RESTAURANT_SUBSIDY_EUR = toPositiveEur(
  process.env.NEXT_PUBLIC_LOYALTY_ARTICLE_SUBSIDY_EUR,
  3
);
...
export const LOYALTY_REWARDS_CATALOG = [
  {
    ...
    name: 'Article offert',
    description:
      'Un dessert ou une boisson non alcoolisée au choix offerts avec la commande. Le partenaire est compensé par CVN\'EAT (plafond article offert).',
    cost: 100,
    icon: '🎁',
    available: true,
    featured: true,
