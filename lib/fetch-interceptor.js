...
if (typeof window !== 'undefined' && originalFetch) {
  ...
  window.fetch = async function(input, init = {}) {
    // IMPORTANT: Ne PAS intercepter les appels Supabase (supabase.co)
    const inputStr = typeof input === 'string' ? input : (input?.url || '');
    if (inputStr.includes('supabase.co') || inputStr.includes('supabase')) {
      // Laisser passer les appels Supabase sans modification
