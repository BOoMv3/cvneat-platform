...
import { formatDateFR, formatEUR } from './restaurant-invoice';
...
export function buildRestaurantTransferInvoicePdfBuffer({
  ...
  return new Promise((resolve, reject) => {
    ...
    const transferAmount = round2(parseFloat(transfer?.amount || 0) || 0);
    const payoutDelta = round2(transferAmount - round2(totals?.totalPayoutDue || 0));
    const showPayoutAdjustment = transferAmount > 0 && Math.abs(payoutDelta) > 0.01;
    const footerPayoutEur = showPayoutAdjustment ? transferAmount : round2(totals?.totalPayoutDue || 0);
    ...
    doc.text('Totaux commandes', cols.order, y);
    ...
    if (showPayoutAdjustment) {
      doc.font('Helvetica').fontSize(10);
      doc.text('Ajustement (alignement virement)', cols.commission - 40, y, { width: 120, align: 'right' });
      doc.text(formatEUR(payoutDelta), cols.payout, y, { width: 75, align: 'right' });
      y += 16;
    }

    doc.font('Helvetica-Bold').fontSize(10);
    doc.text('Net viré (référence paiement)', cols.commission - 40, y, { width: 120, align: 'right' });
    doc.text(formatEUR(footerPayoutEur), cols.payout, y, { width: 75, align: 'right' });
    y += 24;
