...
import { formatDateFR, formatEUR, CVNEAT_INVOICE_PARTY_LINES } from './restaurant-invoice.js';
import { prepareLogoForPdf } from './invoice-pdf-logo.js';
...
export async function buildRestaurantTransferInvoicePdfBuffer({
  restaurant,
  transfer,
  orders,
  totals,
  ...
  const logoJpeg = await prepareLogoForPdf(options.logoBuffer);
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica-Bold').fontSize(16).text('Relevé de ventes et reversement', 50, y);
    y += 22;
    ...
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#6b7280').text('ÉMETTEUR (RESTAURANT)', 50, y);
    doc.text('CLIENT / PLATEFORME', 300, y);
    doc.fillColor('#000000');
    y += 14;
    ...
    if (logoJpeg) {
      try {
        doc.image(logoJpeg, 300, blockStartY, { height: 44 });
        clientYOffset = 52;
      } catch (e) {
        console.warn('Logo PDF ignoré:', e?.message || e);
      }
    }
    clientLines.forEach((line, i) =>
      doc.text(line, 300, blockStartY + clientYOffset + i * 13, { width: 245 })
    );
    y =
      blockStartY +
      Math.max(issuerLines.length * 13 + 4, clientYOffset + clientLines.length * 13 + 4) +
      12;
    ...
    doc.text('Commission CVN\'EAT', cols.commission, y, { width: 70, align: 'right' });
    ...
    doc.font('Helvetica').fontSize(8).fillColor('#6b7280').text(
      "Relevé émis par le restaurant partenaire — reversement du net après commission CVN'EAT. Exonération TVA CVN'EAT, art. 293 B du CGI.",
      50,
      y,
      { width: 500 }
    );
